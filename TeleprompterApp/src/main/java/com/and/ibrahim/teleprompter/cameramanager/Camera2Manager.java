package com.and.ibrahim.teleprompter.cameramanager;
...
//            previewSize = configs.getOutputSizes(SurfaceTexture.class)[0];
    ...
    private final CameraDevice.StateCallback stateCallback = new CameraDevice.StateCallback() {
        ...
        public void onOpened(CameraDevice camera) {
            //This is called when the camera is open
             Log.d(TAG, "onOpened == " + cameraOrientation + ", " + surfaceTexture);
            cameraDevice = camera;
    ...
    private void createCameraPreview() {
        try {
             Log.d(TAG, "previewSize.x = "+previewSize.x+", previewSize.y = "+previewSize.y);
    ...
    CameraCaptureSession.StateCallback captureSessionStateCallback = new CameraCaptureSession.StateCallback(){
        ...
        public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSes) {
            ...
            try {
             ...
             Log.d(TAG, "isStopCamera = "+camView.isStopCamera());
    ...
    public void setResolution(int width, int height) {
        ...
        if(videoFrag != null) {
             savedPreview = SharedPrefManager.getInstance(videoFrag.getActivity()).getPreviewResolution();
        }
        /*else{
        ...
        if(savedPreview != null && !savedPreview.equals("")){
            ...
            VIDEO_WIDTH = Integer.parseInt(tokenizer.nextToken());
             Log.d(TAG, "Saved VIDEO_WIDTH = "+VIDEO_WIDTH );
            VIDEO_HEIGHT = Integer.parseInt(tokenizer.nextToken());
             Log.d(TAG, "Saved VIDEO_HEIGHT = "+VIDEO_HEIGHT );
        }
        else{
            ...
            SharedPrefManager.getInstance(videoFrag.getActivity()).setPreviewResolution(resolutions.append(String.valueOf(VIDEO_WIDTH)).append(",").append(String.valueOf(VIDEO_HEIGHT)).toString());
        }
         Log.d(TAG,"HEIGHT == "+VIDEO_HEIGHT+", WIDTH == "+VIDEO_WIDTH);
